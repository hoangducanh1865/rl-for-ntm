...
def tweets2011():
    filename = 'src/datasets/raw/texts/Tweet.txt'
    save_name = 'src/datasets/pickles/tweets2011'
...
def stackoverflow():
    filename = 'src/datasets/raw/texts/StackOverflow.txt'
    save_name = 'src/datasets/pickles/stackoverflow'
    create_dataset = CreateDataset(data_type='text',
                                   train_file=filename,
                                   test_file=None,
                                   save_name=save_name,
...
def googlenews():
    filename = 'src/datasets/raw/texts/GoogleNews.txt'
    save_name = 'src/datasets/pickles/googlenews'
    create_dataset = CreateDataset(data_type='text',
                                   train_file=filename,
                                   test_file=None,
                                   save_name=save_name,
...
def nytcorpus():
    filename = 'src/datasets/raw/nyt/nyt_output.txt'
    save_name = 'src/datasets/pickles/nyt_corpus_no_stopwords'
    create_dataset = CreateDataset(data_type='text',
                                   train_file=filename,
                                   test_file=None,
                                   save_name=save_name,
...
def contrastive_data():
    ...
    for dataset, vocab_size in zip(datasets, vocab_sizes):
        save_name = f'src/datasets/pickles/contrastive_{dataset}'
...
def ntm_data():
    for dataset in ['20news', 'snippets', 'w2e', 'w2e_text']:
        filename = f'src/datasets/raw/texts/{dataset}_preprocessed_data.txt'
        save_name = f'src/datasets/pickles/ntm_{dataset}_fullvocab'
...
def dbpediasample():
    filename = 'src/datasets/raw/texts/dbpedia_sample_abstract_20k_unprep.txt'
    save_name = 'src/datasets/pickles/wiki20k'
    create_dataset = CreateDataset(data_type='text',
                                   train_file=filename,
                                   test_file=None,
                                   save_name=save_name,
...
def twentynewsgroups(min_word_length):
    if min_word_length == 1:
        save_name = 'src/datasets/pickles/20newsgroups'
    else:
        save_name = f'src/datasets/pickles/20newsgroups_mwl{min_word_length}'
...
class CreateDataset:
    ...
    def get_documents(self):
        ...
        # clntm data sets
            ...
            with open(f'src/datasets/raw/clntm/scholar_data/data/{subfolder}/train.jsonlist') as f:
                train_list = f.readlines()
            train_documents = [json.loads(doc)['text'] for doc in train_list]

            if self.test_file is not None:
                with open(f'src/datasets/raw/clntm/scholar_data/data/{subfolder}/test.jsonlist') as f:
...
class FrozenMapDataPipeBoth(Dataset):
...
class FrozenMapDataPipeBow(Dataset):
...
class ThawedMapDataPipe(Dataset):
