...
from src.utils.utils import create_dir_recursive, get_save_num, get_valid_splits
    ...
    def save_step(self, output_dict, split, epoch, it):
        ...
        def to_float(x):
            if torch.is_tensor(x):
                # detach to avoid autograd tracking, move to cpu, convert to python float
        ...
        self.save_dict[epoch][split][it] = {
            'loss': to_float(output_dict['loss']),
            'coherence': to_float(output_dict['coherence']),
            'diversity': to_float(output_dict['diversity']),
            'perplexity': to_float(output_dict['perplexity'])
        }
...
def save_training_outputs(experiment_name, exp_num, outputs):
    ...
    for epoch in outputs.save_dict.keys():
        ...
        for split in splits:
            for metric in metrics:
                values = [outputs.save_dict[epoch][split][it][metric]
                          for it in outputs.save_dict[epoch][split].keys()]
                # values now plain floats
                epoch_mean = float(np.mean(values)) if values else np.nan
